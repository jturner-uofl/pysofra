#!/usr/bin/env python3
"""Render examples/jss_case_study/jss_case_study.ipynb → HTML.

Run this whenever the case-study notebook source changes:

    .venv/bin/python scripts/render_case_study.py

The script:

1. Renders the *pre-executed* ``jss_case_study.ipynb`` to
   ``jss_case_study.html`` (no re-execution — NHANES download would
   take 20-30 min; re-run the full CI pipeline for that).
2. Injects the same polished CSS overlay used by render_tutorial.py so
   both public HTML artifacts look identical in style.
...
NB   = REPO / "examples" / "jss_case_study" / "jss_case_study.ipynb"
HTML = REPO / "examples" / "jss_case_study" / "jss_case_study.html"
PYTHON  = REPO / ".venv" / "bin" / "python"
JUPYTER = REPO / ".venv" / "bin" / "jupyter"


OVERLAY_CSS = """
<style>/* pysofra-case-study-polish */
...
/* Blockquotes — used for contract assertions and section callouts */
blockquote,
...
/* Markdown tables (scope table, contract index, summary table) */
...
.jp-MarkdownOutput tr:last-child td,
.jp-RenderedMarkdown tr:last-child td {
  border-bottom: none;
}
...
/* Hide cell input/output prompts for a clean publication look. */
.jp-InputPrompt,
.jp-OutputPrompt {
  display: none;
}
...
/* Plain-text output (ASSERTION OK / AUDIT COMPLETE lines, etc.) */
...
/* Highlight ASSERTION OK / AUDIT COMPLETE lines in green. */
.jp-OutputArea-output pre {
  color: #1a202c;
}

/* PySofra tables are self-styled; just give them breathing room. */
table.pysofra {
  margin: 1em 0 !important;
}

/* Image outputs (plots, KM curves, etc.) */
.jp-OutputArea img,
...
/* Strong override: markdown text must always be high-contrast. */
.jp-Cell .jp-RenderedMarkdown,
.jp-Cell .jp-MarkdownOutput {
  color: #1a202c !important;
}

/* Contract-status callout: lines containing ASSERTION OK get a
 * subtle green tint; AUDIT COMPLETE gets a prominent green banner. */
</style>
<style>/* audit-callout — highlight pass/fail lines in stdout */
/* Wrap ASSERTION OK in light-green; AUDIT COMPLETE in strong green banner.
 * This is purely cosmetic — the semantic assertions live in the notebook. */
</style>
"""
...
def main() -> int:
    ...
    # Render to HTML with embedded images (no re-execution — notebook is
    # pre-executed and NHANES download takes ~30 min in CI).
    run([
